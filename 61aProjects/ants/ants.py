...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        #self.entrance,self.exit = self.exit,exit
        if exit != None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        bees = None
        counter = 0
        while not bees and place.is_hive == False:
            if self.lower_bound <= counter <= self.upper_bound:
                if random_bee(place.bees) != None:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for a in place.bees[:]:
            if self.health <= 0:
                a.reduce_health(amount + self.damage)
            else:
                a.reduce_health(amount)
        # END Problem 5
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while place.exit:
            if place.ant != None:
                place.ant.double()
                if place.ant.is_container == True and place.ant.ant_contained:
